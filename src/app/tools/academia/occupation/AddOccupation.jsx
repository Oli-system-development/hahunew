...
const AddOccupation = ({ addOccupation }) => {
  ...
  const onSubmit = async (data) => {
    ...
    levelData.append("levelName", data.levelName);
    levelData.append("levelId", data.levelId);
    levelData.append("levelDescription", data.levelDescription);
    // additionalInputs.forEach((input, index) => {
    //   levelData.append(`ucs[${index}]`, input);
    // });
    // additionalInputs1.forEach((input, index) => {
    //   levelData.append(`instructor[${index}]`, input);
    // });
    ...
    try {
      ...
      console.log("Occupation data submitted successfully");
    } catch (error) {
      console.error(
        "There was an error submitting the occupation data!",
        error
      );
    }
  };
    ...
    <>
      ...
      <div className='flex w-full flex-col md:flex-row gap-2 md:shadow-xl rounded-xl  md:p-4 '>
        <form
          ...
          className='flex w-full flex-col md:flex-row gap-2 md:shadow-xl rounded-xl  md:p-4 '
        >
